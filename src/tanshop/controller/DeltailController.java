...
public class DeltailController {
	...
	public String typePro(ModelMap model, HttpServletRequest  request, @PathVariable("id") String id){
		try {
			...
			System.out.print(list1.get(0).getGroupProduct().getId());
			...
			int type;
			if (list1.get(0).getGroupProduct().getId().contains("v")) {
				type = 1;
			}
			else if (list1.get(0).getGroupProduct().getId().contains("g")) {
				type = 2;
			}
			else if (list1.get(0).getGroupProduct().getId().contains("aq")) {
				type = 3;
			}
			else {
				type = 4;
